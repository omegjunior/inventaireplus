...
/**
 * \file    inventaireplus/admin/setup.php
 * \ingroup inventaireplus
 * \brief   InventairePlus setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "inventaireplus@inventaireplus"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('inventaireplussetup', 'globalsetup'));
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/inventaireplus/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=inventaireplus-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'INVENTAIREPLUS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'INVENTAIREPLUS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "InventairePlusSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-inventaireplus page-admin');
...
$head = inventaireplusAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "inventaireplus@inventaireplus");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'INVENTAIREPLUS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'INVENTAIREPLUS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
