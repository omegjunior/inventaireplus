...
$langs->loadLangs(array('products', 'stocks', 'orders', 'inventaireplus@inventaireplus'));
...
//spécifier le contexte pouur inventaireplus
$contextpage .= 'InventairePlus';
$id = GETPOSTINT('id');
...
// Security check
if (!$user->rights->inventaireplus->stock->create) {
	accessforbidden();
}
...
//particularité InventairePlus
$usercancreatestock = $user->hasRight('inventaireplus', 'stock', 'create');
...
if ($action == "correct_stock" && $permissiontoadd) {
	...
	if (!$error) {
		...
		if ($result > 0) {
			header("Location: ".$_SERVER["PHP_SELF"]."?id=".$id.'&action=correction');
			exit;
		} else {
			$error++;
...
if ($object->id > 0) {
	...
	if (!empty($usercancreatestock)) {
	  $head[$h][0] = DOL_URL_ROOT.'/custom/inventaireplus/product/stock/movement_list.php?id='.$object->id.'&action=correction';
	  $head[$h][1] = $langs->trans("WarehouseInventairePlus");
	  $head[$h][2] = 'movements';
	  $h++;
	}

	print dol_get_fiche_head($head, 'movements', $langs->trans("WarehouseInventairePlus"), -1, 'stock');


	$linkback = '<a href="'.DOL_URL_ROOT.'/custom/inventaireplus/product/stock/movement_list.php?id='.$id.'&action=correction">'.$langs->trans("BackToList").'</a>';
...
if ($action == "correction") {
	include DOL_DOCUMENT_ROOT.'/custom/inventaireplus//product/stock/tpl/stockcorrection.tpl.php';
	print '<br>';
}
