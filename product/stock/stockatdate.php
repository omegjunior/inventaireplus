...
$sql .= $hookmanager->resPrint;
//éliminer les lignes avec 0 pour inventaireplus
$sql .= ' HAVING NOT (sellvalue = 0  AND estimatedvalue = 0) ';
...
if ($ext == 'csv') {
	...
	$head[0][0] = DOL_URL_ROOT.'/custom/inventaireplus/product/stock/stockatdate.php';
	$head[0][1] = $langs->trans("StockAtDateInPast");
	$head[0][2] = 'stockatdatepast';

	$head[1][0] = DOL_URL_ROOT.'/custom/inventaireplus/product/stock/stockatdate.php?mode=future';
...
while ($i < ($limit ? min($num, $limit) : $num)) {
	...
	if (getDolGlobalString('STOCK_SUPPORTS_SERVICES') || $objp->fk_product_type == 0) {
		...
		if ($ext == 'csv') {
			if ($mode == 'future') {
				...
				print implode(";", array(
					...
					price2num($objp->sellvalue, 'MT') ? '"'.price2num($objp->sellvalue, 'MT').'"' : '',
					"$nbofmovement",
					'"'.price2num($currentstock, 'MS').'"'))."\r\n";
				$totalbuyingprice += $stock * $objp->pmp;
				$totalsellingprice += (float) $objp->sellvalue;
			}
		...
		if ($mode == 'future') {
			...
			if (price2num($objp->sellvalue, 'MT')) {
				print '<span class="amount">'.price(price2num($objp->sellvalue, 'MT'), 1).'</span>';
			}
			$totalsellingprice += (float) $objp->sellvalue;
			print '</td>';

			// Movements since date
			print '<td class="right">';
			if ($nbofmovement > 0) {
...
if ($ext == 'csv') {
	print implode(
		...
		array(
			...
			'"'.price2num($totalsellingprice, 'MT').'"',
			'',
			$productid > 0 ? price2num($totalcurrentstock, 'MS') : '')
	);
} else {
