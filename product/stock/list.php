...
/**
 *      \file       htdocs/custom/inventaireplus/product/stock/list.php
 *      \ingroup    stock
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'stocklistInventairePlus'; // To manage different context of search
...
$valuationSqlParts = inventaireplusGetWarehouseValuationSqlParts($conf);
$separatedPMP = !empty($valuationSqlParts['separatedPMP']);
$sql .= ", ".$valuationSqlParts['select'];
	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields["estimatedvalue"]['checked'])) {
			...
			if (price2num($obj->estimatedvalue, 'MT')) {
				...
				print '<span class="amount">0</span>';
			}
			print '</td>';
			if (!$i) {
				$totalarray['nbfield']++;
		...
		if (!empty($arrayfields["estimatedstockvaluesell"]['checked'])) {
			...
			//if (!getDolGlobalString('PRODUIT_MULTIPRICES')) {
				if ($obj->sellvalue) {
					// print price or 0 if price is empty. This is to avoid showing empty value when price is 0, but show 0 when price is really 0.
...
$hasWarehouseValuationMassAction = $user->hasRight('stock', 'creer');
if ((in_array('builddoc', array_keys($arrayofmassactions)) || $hasWarehouseValuationMassAction) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	$hidegeneratedfilelistifempty = 1;
	if ($massaction == 'builddoc' || $massaction == 'builddocwarehousevaluationinventaireplus' || $action == 'remove_file' || $show_files) {
