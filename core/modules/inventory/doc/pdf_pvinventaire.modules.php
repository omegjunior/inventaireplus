...
/**
 *  \file       inventaireplus/core/modules/inventory/doc/pdf_pvinventaire.modules.php
 *  \ingroup    inventory
 *  \brief      PDF model for inventory minutes
 */
...
class pdf_pvinventaire extends ModelePDFFactures
{
	public $db;
	public $name;
	public $description;
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = 'pvinventaire';
		$this->description = 'PV d’inventaire';
		$this->type = 'pdf';
	...
	public function write_file($parameters, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($inventoryId <= 0 || empty($dir)) {
			$this->error = 'Paramètres incomplets pour générer le PV d\'inventaire.';
			return 0;
		}

		$dataset = inventaireplusBuildInventoryDocumentDataset($this->db, $inventoryId, false);
		if (empty($dataset['lines'])) {
			$this->error = 'Aucune ligne n\'est disponible pour le PV d\'inventaire.';
			return 0;
		}
		...
		$filename = 'pv_inventaire_'.$inventoryRefSafe.'.pdf';
		$file = $dir.'/'.$filename;
		...
		$pdf->SetTitle($outputlangs->convToOutputCharset('PV D\'INVENTAIRE '.(!empty($dataset['context']['inventory_ref']) ? $dataset['context']['inventory_ref'] : $inventoryId)));
		$pdf->SetSubject($outputlangs->convToOutputCharset('PV d\'inventaire'));
		...
		foreach ($dataset['categories'] as $category) {
			...
			foreach ($category['lines'] as $line) {
				...
				$cells = array(
					...
					(string) $line['qty_theoretical'],
					(string) $line['qty_physical'],
			...
			$pdf->SetXY($x, $y);
			$pdf->MultiCell($this->cols[5]['width'], 7, (string) $category['total_theoretical'], 1, 'R', false, 0, '', '', true, 0, false, true, 7, 'M');
			$x += $this->cols[5]['width'];
			$pdf->SetXY($x, $y);
			$pdf->MultiCell($this->cols[6]['width'], 7, (string) $category['total_physical'], 1, 'R', false, 0, '', '', true, 0, false, true, 7, 'M');
			$x += $this->cols[6]['width'];
			$pdf->SetXY($x, $y);
			$pdf->MultiCell($this->cols[7]['width'], 7, (string) $category['total_delta'], 1, 'R', false, 0, '', '', true, 0, false, true, 7, 'M');
			$x += $this->cols[7]['width'];
			$pdf->SetXY($x, $y);
	...
	protected function _pagehead(&$pdf, $context, $outputlangs)
	{
		...
		$pdf->MultiCell($titleWidth, 4, 'PV D\'INVENTAIRE', 0, 'R');
